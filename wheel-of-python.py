...
class WOFPlayer:
    ...
    def addMoney(self, amt):
        try: self.prizeMoney += amt
        except: self.prizeMoney += 0
    def goBankrupt(self):
        self.prizeMoney = 0
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        for lt in LETTERS:
            if lt in guessed:
                continue
            elif (lt not in guessed) and (self.prizeMoney < VOWEL_COST)and (lt in VOWELS):
                continue
            else: accum = accum + lt
        return accum
        
    def getMove(self, category, obscuredPhrase, guessed):
        pos = self.getPossibleLetters(guessed)
        if len(pos) == 0:
            return 'pass'
        
        coinFlip = self.smartCoinFlip()
        letters = [lt for lt in pos]
        if coinFlip == True:
            pos_freq = list(filter(lambda x: x in letters,self.SORTED_FREQUENCIES))
            return pos_freq[-1]
        else:
